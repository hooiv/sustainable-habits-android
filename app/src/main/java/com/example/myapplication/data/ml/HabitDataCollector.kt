...
class HabitDataCollector @Inject constructor(
    ...
    suspend fun collectTrainingData(habitId: String): List<Pair<FloatArray, FloatArray>> {
        ...
        try {
            ...
            val habit = habitDao.getHabitById(habitId).first()
            if (habit == null) {
    ...
    private fun generateTrainingDataFromCompletions(
        ...
        val completionRate = if (habit.creationDate != null) {
            val daysSinceCreation = (System.currentTimeMillis() - habit.creationDate) / (1000 * 60 * 60 * 24)
            if (daysSinceCreation > 0) completions.size.toFloat() / daysSinceCreation else 0f
            } else 0f
        ...
        for (i in 1 until sortedCompletions.size) {
            ...
            features[FEATURE_DIFFICULTY] = habit.difficulty.toFloat() / 5f
